...
SELECT name as name_of_user,YEAR(date_of_birth) as birth_year,MONTH(date_of_birth) as month_of_birth,DAY(date_of_birth) as day_of_birth from users;

-- DATEDIFF()
-- Find number of days between today and birthdate:
SELECT DATEDIFF(NOW(),date_of_birth) as number_of_days_lived from users;
SELECT DATEDIFF(CURDATE(),date_of_birth) as number_of_days_lived from users;
-- In DATEDIFF(), both queries will give the same number_of_days_lived...IN NORMAL NOW() GIVE date and time both however CURDATE() gives only current date

-- TIMESTAMPDIFF()
-- Calculate age in years:
SELECT name, TIMESTAMPDIFF(YEAR,date_of_birth,CURDATE()) as age_in_year from users;
-- Calculate age in months:
SELECT name, TIMESTAMPDIFF(MONTH,date_of_birth,CURDATE()) as age_in_month from users;
...
-- Returns each user’s ID with the remainder after dividing by 2, where 1 = odd ID and 0 = even ID.
